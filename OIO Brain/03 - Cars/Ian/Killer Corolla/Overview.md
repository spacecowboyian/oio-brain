---
title: Killer Corolla — 1977 Toyota Corolla Sport Coupe
type: vehicle
status: active
owner: Ian Jennings
updated: 2026-03-27
tags: [vehicle, corolla, killer-corolla, down, rebuild, trd]
source_of_truth: true
summary: Overview of the Killer Corolla — Ian's 1977 Corolla Sport Coupe loaded with vintage TRD parts. Down since ~2021 after dropping a valve mid-autocross. Six-year resurrection underway with a $100 mystery motor.
---

# Killer Corolla — 1977 Toyota Corolla Sport Coupe

> Six-year resurrection. Loaded with vintage TRD parts. $100 mystery motor.

*Last updated: March 2026*

---

## Quick Reference

| Field | Value |
|---|---|
| Nickname | Killer Corolla |
| Year / Make / Model | 1977 Toyota Corolla Sport Coupe |
| Driver | Ian Jennings |
| Status | **Down** — rebuild in progress |
| Primary Use | Autocross (originally), TBD post-rebuild |
| 2026 Arc | Six-year resurrection. $100 mystery motor. TRD everything eventually. |

---

## Background

A 1977 Toyota Corolla Sport Coupe loaded with vintage TRD parts. Dropped a valve mid-autocross approximately five years ago. Has sat since. The current plan involves a $100 mystery motor and a very long list of TRD components.

The resurrection arc is one of the longest-running stories in OIO history. The car has been down for six years. 2026 is when it starts coming back.

---

## The $100 Mystery Motor

Ian acquired a mystery motor for $100. Whether it runs, what exactly it is, and whether it belongs in this car are all open questions. This is documented content.

---

## TRD Parts Collection

The Killer Corolla is loaded with vintage TRD (Toyota Racing Development) parts. Part of the appeal is that it's a period-correct Toyota race car, not a modified street car.

*Document TRD parts inventory in Setup Notes.*

---

## 2026 Story Arc

Six years down. The rebuild begins. $100 mystery motor. TRD everything eventually.

The question is not if — it's when and how bad it gets before it runs.

---

## Related Content

→ [Setup Notes](Setup-Notes.md)
→ [Maintenance Log](Maintenance-Log.md)

---

## Related Videos

| Date | Title | Views | Format |
|---|---|---|---|
| 2025-12-15 | [killer corolla 2tg #cars #toyota #vintagecars](https://www.youtube.com/watch?v=OJtWWOQo4iM) | 1,589 views | General |
| 2024-01-12 | [Killer Corolla #TE51 #2TG #toyota #shorts](https://www.youtube.com/watch?v=tOJtKLGYWX8) | 4,435 views | Race Event |
| 2023-12-10 | [Soon. #killercorolla #te51 #corolla #toyota #autocross](https://www.youtube.com/watch?v=hFTIkB18yrg) | 305 views | Race Event |
| 2020-07-30 | [1977 Killer Corolla - Vintage Toyota Racing Disaster!](https://www.youtube.com/watch?v=fnQbi6dzDK8) | 951 views | How-To / DIY |
| 2019-10-08 | [1976 TE51 "Killer" Corolla Autocross](https://www.youtube.com/watch?v=LXqGcQL5_zs) | 146 views | Race Event |

*5 videos total. Views as of March 2026.*