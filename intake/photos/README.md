...
This is the intake zone for photos that need to be added to the OIO photo library. It works the same way as `intake/docs/` — it's a processing queue, not a permanent home.
...
4. The Copilot agent:
   ...
   - Updates the car's `Overview.md` in `cars/` with any new visual detail
...
- If a photo cannot be identified, the agent will add it to `active/open-loops.md` and leave it here with a note.
