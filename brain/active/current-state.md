...
- **2026-04-13:** `transcripts/` moved into `brain/transcripts/` — aligns transcript archive with brain structure. All scripts (`fetch_transcripts.py`, `clean_transcripts.py`, `update_catalog_transcripts.py`), `clean-transcripts.yml` workflow, `README.md`, `brain/skill.md`, `.github/copilot-instructions.md`, and `docs/transcript-pipeline.md` updated to reference new path.
- **2026-03-29:** Transcript system clarified — two types distinguished: (1) final YouTube video transcripts (`brain/transcripts/`, auto-fetched) and (2) raw dailies (`intake/dailies/`, manual intake). `intake/docs/transcripts/` renamed to `intake/dailies/`. `process-transcripts.yml` replaced with `process-dailies.yml`. Rogue daily `failedfit lutchbleeddrive.txt` processed and deleted — summary filed at `content/summaries/2026-03-29_failed-fit-clutch-bleed-drive.md`. Honda Fit Maintenance-Log.md created.
