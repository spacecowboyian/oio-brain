...
- Any new photos that haven't already been processed are downloaded and dropped into the `intake/photos/` folder.
- The action commits the new images to the repo so the existing intake/photos processing workflow can pick them up.

### 2. AI Filing Agent

- The existing intake/photos processing agent attempts to identify each photo: what car, what event, what driver, what context.
...
- Caption voice must follow the OIO Brand Guide (see `brand/`).
